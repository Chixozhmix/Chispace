...
public class ModBiomeModifirs {
    ...
    public static final ResourceKey<BiomeModifier> ADD_COLD_OAK = registerKey("add_cold_oak");
    ...
    public static void bootstrap(BootstapContext<BiomeModifier> context) {
        ...
        context.register(ADD_SOUL_GEM_ORE, new ForgeBiomeModifiers.AddFeaturesBiomeModifier(
                HolderSet.direct(biomes.getOrThrow(ModBiomes.COLD_PLAINTS)), // Прямое указание биома
                HolderSet.direct(placedFeature.getOrThrow(ModPlacedFeature.SOUL_GEM_ORE_PLACED_KEY)),
                GenerationStep.Decoration.UNDERGROUND_ORES));

        context.register(ADD_COLD_OAK, new ForgeBiomeModifiers.AddFeaturesBiomeModifier(
                biomes.getOrThrow(Tags.Biomes.IS_PLAINS),
                HolderSet.direct(placedFeature.getOrThrow(ModPlacedFeature.COLD_OAK_KEY)),
